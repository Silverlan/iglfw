	...
	glfwSetJoystickCallback([](int joystickId,int eventId) {
		...
		switch(eventId)
		{
			case GLFW_CONNECTED:
				...
				break;
			default:
				auto it = std::find_if(handler->m_joysticks.begin(),handler->m_joysticks.end(),[joystickId](const std::shared_ptr<Joystick> &joystick) {
					return (joystick->GetJoystickId() == joystickId) ? true : false;
				});
